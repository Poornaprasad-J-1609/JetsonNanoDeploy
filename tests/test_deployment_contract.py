...
def test_policy_walking_has_no_hardcoded_gait_target_substitution():
    ...
    motion_assist_cfg = load_yaml(ROOT / "config" / "motion_assist.yaml")

    assert "def apply_gait_assist" not in controller_source
    assert "suspension_preview" not in controller_source
    assert "gait_assist" not in motion_assist_cfg
    assert not (ROOT / "scripts" / "run_suspension_gait_preview.sh").exists()
...
def test_policy_entry_uses_official_policy_gains_while_target_ramps():
    ...
    for target_scale in np.linspace(0.0, 1.0, 11):
        q_target[joint_index] = 0.20 * float(target_scale)
        command = layer.build_mit_commands(
            q_target,
            phase="policy",
            feedback_by_joint=feedback,
...
def test_policy_entry_policy_gains_remain_inside_policy_torque_limit():
    ...
    for target_scale in np.linspace(0.0, 1.0, 101):
        q_target[joint_index] = 0.40 * float(target_scale)
        command = layer.build_mit_commands(
            q_target,
            phase="policy",
            feedback_by_joint=feedback,
